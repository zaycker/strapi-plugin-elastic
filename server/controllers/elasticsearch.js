...
module.exports = {
  ...
  fetchModel: async (ctx) => {
    ...
    try {
      count = await strapi.$es.count({ index });
      map = await strapi.$es.indices.getMapping({ index });
      status = {
        deleted: false,
        created: true,
      };
  ...
  migrateModel: async (ctx) => {
    ...
    await strapi.$es.migrateModel(model);
  ...
  createIndex: async (ctx) => {
    ...
    await strapi.$es.indices.create(options);
  ...
  deleteIndex: async (ctx) => {
    ...
    try {
      await strapi.$es.indices.delete({
        index: targetModel.index,
      });
      return ctx.send({ success: true });
    } catch (e) {
