...
import { Typography, ProgressBar, Select } from '@strapi/design-system';
...
const DataView = ({
  ...
  const tableHeaders = useMemo(
    () => (data && data.length ? Object.keys(data[0]).map((d) => ({ name: d, value: d })) : []),
    [data]
  );
  ...
  const migrate = (model) => {
    setIsMigrating(true);
    request(`/elasticsearch/migrate-model`, {
      method: 'POST',
      body: { model },
    })
      .then((res) => {
  ...
  const deleteIndex = (model) => {
    setIsDeleting(true);
    request(`/elasticsearch/delete-index`, {
      method: 'POST',
      body: { model },
    })
      .then((res) => {
  ...
  const createIndex = (model) => {
    setIsCreating(true);
    request(`/elasticsearch/create-index`, {
      method: 'POST',
      body: { model },
    })
      .then((res) => {
